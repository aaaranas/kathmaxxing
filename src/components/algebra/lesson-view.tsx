...
/** The jump list at the top of a lesson, so a long page stays navigable. */
export function LessonContents({ lesson }: { lesson: Lesson }) {
  return (
    <nav aria-label="On this page" className="mb-4 rounded-lg border border-line bg-desk px-4 py-3">
      ...
      <ul className="flex flex-wrap gap-1.5">
        {lesson.sections.map((section) => (
          <li key={section.id}>
            <a
              href={`#${section.id}`}
              className="inline-block rounded-md border border-line bg-paper px-2.5 py-1 text-sm hover:bg-plate"
            >
              {section.title}
            </a>
          </li>
